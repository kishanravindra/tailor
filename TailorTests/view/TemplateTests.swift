...
@available(*, deprecated) class TemplateTests: TailorTestCase {
  ...
  func testLinkPutsLinkTagInBuffer() {
    ...
    RouteSet.load {
      routes in
      routes.route(.Get("test/path"), to: InnerTestController.indexAction, name: "index")
      ()
    }
    
    template2.link(controllerName: InnerTestController.name, actionName: "index", parameters: ["id": "5"], attributes: ["class": "btn"]) {
