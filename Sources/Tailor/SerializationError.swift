/**
 This enum holds errors that can occur when parsing data, or extracting
 values from a serializable primitive.
 */
public enum SerializationParsingError : ErrorType, Equatable {
  /**
  ...
  /**
   This error occurs when trying to read a value from a primitive when the
   primitive is not of the desired type.
   
   This includes a field name so that if it occurs deep in parsing a nested
   data structure, we can build up the field name to tell where in the
   ...
   - parameter caseType: The type that the primitive wraps around.
   */
  case WrongFieldType(field: String, type: Any.Type, caseType: Any.Type)
  
  /**
   This error occurs when trying to read a field from a dictionary that is
   not present.
  ...
  public static func withFieldPrefix<T>(prefix: String, block: Void throws->T) throws -> T {
    ...
    catch WrongFieldType(field: let field, type: let type, caseType: let caseType) {
      throw WrongFieldType(field: "\(prefix).\(field)", type: type, caseType: caseType)
    }
    catch MissingField(field: let field) {
      throw MissingField(field: "\(prefix).\(field)")
    }
    catch let e {
      throw e
    }
 ...
 */
public func ==(lhs: SerializationParsingError, rhs: SerializationParsingError) -> Bool {
  switch(lhs) {
 ...
 */
public enum SerializationConversionError: ErrorType, Equatable {
  /**
   This error is thrown when trying to get data from a foundation object
